...
impl fmt::Display for DeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match self {
            ...
            DeError::CellError { ref err } => {
                write!(f, "Cell error: {}", err)
            }
            ...
            DeError::Positional { ref err, ref pos } => {
    ...
    D: DeserializeOwned,
{
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let remaining = if self.end_pos.0 > self.current_pos.0 {
            (self.end_pos.0 - self.current_pos.0) as usize
        } else {
            0_usize
        };
    ...
    T: ToCellDeserializer<'cell>,
{
        ...
        D: DeserializeSeed<'de>,
    {
        self.iter
            .next()
            .map(|i| {
                let cell_pos = (self.pos.0, self.pos.1 + *i as u32);
                let de = self.cells[*i].to_cell_deserializer(cell_pos);
                seed.deserialize(de)
            })
    ...
    ($typ:ty, $method:ident, $visit:ident) => {
            ...
            V: Visitor<'de>,
        {
            match self.data_type {
                ...
                Data::String(ref s) => {
                    let v = s.parse().map_err(|_| DeError::Positional {
                        err: Box::new(DeError::Custom(format!(
                            "Expecting {}, got '{}'",
                            stringify!($typ),
                            s
                        ))),
...
impl<'a, 'de> Deserializer<'de> for DataDeserializer<'a> {
        ...
        V: Visitor<'de>,
    {
        match self.data_type {
            ...
            Data::DateTime(v) => visitor.visit_str(
                &v.as_datetime()
                    .unwrap()
                    .format("%Y-%m-%dT%H:%M:%S")
                    .to_string(),
